...
[ freqHEOM, fluorescenceHEOM, freqRC, fluorescenceRC ] = RCFLinstance.getBothFluorescence(paramsRC, RCfolder, HEOMfolder);
...
maxFreq = 2000;
color = [1 0.8 0.2];
[ freqHEOMreduced, fluorescenceHEOMreduced ] =...
    eqPointDistance( freqHEOM./maxFreq, fluorescenceHEOM./max(fluorescenceHEOM), 0.08 );
d = plot( freqHEOMreduced.*maxFreq, fluorescenceHEOMreduced, 'ko', 'MarkerSize', 3,...
    'MarkerFaceColor', color);
grid on;

set(fig, 'Units', 'centimeters');
set(fig, 'Position', [10 4 13 9]);
axis([13000 17000 0 1.1]);
xlabel('\omega, cm^{-1}');
ylabel('Normuota fluorescencija');
legend([a d], 'RC fluorescencija', 'HEOM fluorescencija' );